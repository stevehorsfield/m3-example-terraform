resource "aws_autoscaling_group" "prometheus1" {

  name = "${var.environment}-prometheus1"
  ...
  vpc_zone_identifier = ["${module.integration.private_subnets[1]}"]
  ...
  mixed_instances_policy {
    ...
    launch_template {
      launch_template_specification {
        launch_template_id = "${aws_launch_template.prometheus1.id}"
        version = "$$Latest"
      }
  ...
  tag {
    key                  = "Name"
    value                = "${var.environment}-prometheus1"
...
resource "aws_launch_template" "prometheus1" {
  name = "${var.environment}-prometheus1"
  ...
  user_data = "${base64encode(data.template_file.prometheus1.rendered)}"
}

data "template_file" "prometheus1" {
  ...
  vars = {
    ...
    volume-id         = "${aws_ebs_volume.prometheus1.id}"
    hostname          = "prometheus1.${module.integration.dns-private-forward-zone-name}"
  }
}

resource "aws_ebs_volume" "prometheus1" {
  availability_zone = "us-east-1b"
  size              = "${var.prometheus-configuration["storage-size-gb"]}"
  type              = "gp2"

  tags {
    Name        = "${var.environment}-prometheus1-data"
    Environment = "${var.environment}"
    Application = "prometheus"
    Volume      = "prometheus1-data"
  }
}
