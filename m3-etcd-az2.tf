resource "aws_instance" "m3-etcd2" {
  ami                         = "${var.m3-configuration["etcd-ami"]}"
  private_ip                  = "${cidrhost(element(module.integration.private_fixed_subnet_cidrs,2), 32)}"
  subnet_id                   = "${element(module.integration.private_fixed_subnets,2)}"  # implies AZ
  ...
  user_data                   = "${data.template_file.m3-etcd2-userdata.rendered}"
  ...
  tags = {
    Name        = "${var.environment}-m3-etcd2"
    Environment = "${var.environment}"
    Application = "M3"
    VpcHostname = "m3-etcd2"
...
data "template_file" "m3-etcd2-userdata" {
  ...
  vars {
    ...
    hostname      = "m3-etcd2.${module.integration.dns-private-forward-zone-name}"
...
resource "aws_ebs_volume" "m3-etcd2-data" {
  ...
  tags = {
    Name        = "${var.environment}-m3-etcd2-data"
    Environment = "${var.environment}"
    Application = "M3"
    Volume      = "etcd2-data"
  }
...
resource "aws_volume_attachment" "m3-etcd2-data" {
  device_name = "/dev/xvdb"
  volume_id   = "${aws_ebs_volume.m3-etcd2-data.id}"
  instance_id = "${aws_instance.m3-etcd2.id}"

  skip_destroy = true
}

resource "aws_cloudwatch_metric_alarm" "m3-etcd2-recovery" {
  alarm_name          = "${var.environment}-m3-etcd2-auto-recover"
  namespace           = "AWS/EC2"
  ...
  dimensions {
    InstanceId = "${aws_instance.m3-etcd2.id}"
  }
}