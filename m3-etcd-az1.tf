resource "aws_instance" "m3-etcd1" {
  ami                         = "${var.m3-configuration["etcd-ami"]}"
  private_ip                  = "${cidrhost(element(module.integration.private_fixed_subnet_cidrs,1), 32)}"
  subnet_id                   = "${element(module.integration.private_fixed_subnets,1)}"  # implies AZ
  ...
  user_data                   = "${data.template_file.m3-etcd1-userdata.rendered}"
  ...
  tags = {
    Name        = "${var.environment}-m3-etcd1"
    Environment = "${var.environment}"
    Application = "M3"
    VpcHostname = "m3-etcd1"
...
data "template_file" "m3-etcd1-userdata" {
  ...
  vars {
    ...
    hostname      = "m3-etcd1.${module.integration.dns-private-forward-zone-name}"
...
resource "aws_ebs_volume" "m3-etcd1-data" {
  ...
  tags = {
    Name        = "${var.environment}-m3-etcd1-data"
    Environment = "${var.environment}"
    Application = "M3"
    Volume      = "etcd1-data"
  }
...
resource "aws_volume_attachment" "m3-etcd1-data" {
  device_name = "/dev/xvdb"
  volume_id   = "${aws_ebs_volume.m3-etcd1-data.id}"
  instance_id = "${aws_instance.m3-etcd1.id}"

  skip_destroy = true
}

resource "aws_cloudwatch_metric_alarm" "m3-etcd1-recovery" {
  alarm_name          = "${var.environment}-m3-etcd1-auto-recover"
  namespace           = "AWS/EC2"
  ...
  dimensions {
    InstanceId = "${aws_instance.m3-etcd1.id}"
  }
}