resource "aws_autoscaling_group" "prometheus2" {

  name = "${var.environment}-prometheus2"
  ...
  vpc_zone_identifier = ["${module.integration.private_subnets[2]}"]
  ...
  mixed_instances_policy {
    ...
    launch_template {
      launch_template_specification {
        launch_template_id = "${aws_launch_template.prometheus2.id}"
        version = "$$Latest"
      }
  ...
  tag {
    key                  = "Name"
    value                = "${var.environment}-prometheus2"
...
resource "aws_launch_template" "prometheus2" {
  name = "${var.environment}-prometheus2"
  ...
  user_data = "${base64encode(data.template_file.prometheus2.rendered)}"
}

data "template_file" "prometheus2" {
  ...
  vars = {
    ...
    volume-id         = "${aws_ebs_volume.prometheus2.id}"
    hostname          = "prometheus2.${module.integration.dns-private-forward-zone-name}"
  }
}

resource "aws_ebs_volume" "prometheus2" {
  availability_zone = "us-east-1c"
  size              = "${var.prometheus-configuration["storage-size-gb"]}"
  type              = "gp2"

  tags {
    Name        = "${var.environment}-prometheus2-data"
    Environment = "${var.environment}"
    Application = "prometheus"
    Volume      = "prometheus2-data"
  }
}
